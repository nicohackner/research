using Plots, LinearAlgebra, DataFrames, CSV;
...
function potential()
    ...
    Deltanew = zeros(ComplexF64, Nlat, Nlat);
        ...
        if counter == 0 && iter > 5 
            ...
            CSV.write("delta.csv",  DataFrame(Delta), writeheader=false)
            CSV.write("ham.csv",  DataFrame(Ham), writeheader=false)
            CSV.write("evals.csv",  DataFrame(transpose(evals)), writeheader=false)
            break
        else
